...
scene::IMesh * CSceneManager::getMeshofSceneNode(scene::ISceneNode* sceneNode)
{
	if (!sceneNode)
		return 0;
	scene::IMesh* pMesh;
	scene::ESCENE_NODE_TYPE nodeType = sceneNode->getType();

	if (nodeType == scene::ESNT_MESH)
	{
		scene::IMeshSceneNode * meshNode;
		meshNode = (scene::IMeshSceneNode *)sceneNode;
		pMesh = meshNode->getMesh();
	}

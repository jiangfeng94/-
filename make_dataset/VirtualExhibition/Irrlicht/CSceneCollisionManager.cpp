...
IMeshBuffer* CSceneCollisionManager::getMeshBufferFromRayBB(core::line3d<f32> ray, ISceneNode* root,
	s32 idBitMask,
	bool bNoDebugObjects)
{
	...
	getPickedMeshBufferBB(root, ray,
		idBitMask, bNoDebugObjects, dist, best);

	return best;
}

void CSceneCollisionManager::getPickedMeshBufferBB(ISceneNode* root,
	const core::line3df& ray,
	s32 bits,
	bool bNoDebugObjects,
	f32& outbestdistance,
	IMeshBuffer*& outbestbuffer)

{
	core::vector3df edges[8];
	scene::IMesh * mesh;

	if ((root->getType() != scene::ESNT_MESH) && (root->getType() != scene::ESNT_ANIMATED_MESH))
		return;
	IMeshSceneNode* meshCurrent = (IMeshSceneNode*)root;
	mesh = meshCurrent->getMesh();



	for (s32 it = 0; it < mesh->getMeshBufferCount(); ++it)
	{
		IMeshBuffer* meshbuffer = mesh->getMeshBuffer(it);
		// get world to object space transform
		core::matrix4 mat;
		if (!root->getAbsoluteTransformation().getInverse(mat))
			continue;
		...
		const core::aabbox3df& box = meshbuffer->getBoundingBox();

...
namespace scene
{
	...
	*/
	...
	public:
		...
		virtual void setModelType(const c8* type) {
			Type = type;
		}
		virtual const c8* getModelType()const {
			return Type.c_str();
		}

		ISceneNode* fathermodel = NULL;
		ISceneNode*  getFatherModel() {
			return fathermodel;
		}
		void setFatherModel(ISceneNode* model) {
			fathermodel = model;
		}
		ISceneNode* sonmodel = NULL;
		ISceneNode*  getSonModel() {
			return sonmodel;
		}
		void setSonModel(ISceneNode* model) {
			sonmodel = model;
		}
